...
$(function(){
    ...
    $(document).on('click',"#client_account_types-groups input[type='radio']", function(event){
        if (isAjax) {
            event.preventDefault();
        } else {
            hidePortfolioActionButton();
            showContentInRightBox('<img class="ajax-loader" src="/static/img/ajax-loader.gif" />');
    ...
    $(document).on('click',"#client_account_types-group_type input[type='radio']", function(){
        var form = $(this).closest('form');
            ...
            success: function(response){
                if (response.status == 'success') {
                    ...
                    if (response.step == 1) {
                        enabledLeftBox();
                        showAccountsTable();
                        $('#client_account_types-groups input[type="radio"]:checked').removeAttr('checked');
                    }
...
{
    $('#client_account_types-groups input[type="radio"]').attr('disabled', 'disabled');
}

function enabledLeftBox()
{
    $('#client_account_types-groups input[type="radio"]').removeAttr('disabled');
}
...
{
    $("#client_account_types-groups input[type='radio']").attr('checked', false);
}
